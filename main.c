...
int main(int argc, char *argv[])
{
    ...
    fprintf(stdout, "%s: %s 服务器搭建完成：\n [PID: %d]\n [端口号: %s] [队列长度: %d] \n[线程数: %d]\n", SERVER_NAME, OK_MSG, getpid(), port, queue_length, num_threads);
	fprintf(stdout, "%s: %s help获取具体信息 \n", SERVER_NAME, INFO_MSG);
   fprintf(stdout, "%s: %s stop 退出程序 \n", SERVER_NAME, INFO_MSG);
...
void *p2p(void *args)
{
	...
	while((strcmp(in, "CONNECT")) != 0 && (strcmp(in, "QUIT") != 0))
	{
		...
		{
			fprintf(stdout, "%s: %s  %s 向服务器发送握手消息，返回确认消息 [句柄: %d]\n", SERVER_NAME, OK_MSG,peeraddr, user_fd);
